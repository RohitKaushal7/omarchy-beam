...
function guardLine(index, expression) {
  return "if { " + substituteGuardReaders(expression) + "; } >/dev/null 2>&1; then echo "
    + index + ":w:1; else echo " + index + ":w:0; fi\n"
}

// One bash script answering every `when:` guard as `<n>:w:<0|1>` lines, where
// n indexes `ids`. Menu ids are keys from JSONC files, so they never enter the
// script as text. Beam never shows ✓ markers, so `checked:` guards are not
// evaluated.
function guardScript(items) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    guards += guardLine(ids.length, entry.when)
    ids.push(keys[i])
  }
  if (!guards) return { script: "", ids: [] }
  ...
  return { script: prelude + guards, ids: ids }
}

function parseGuardOutput(text, ids) {
  var results = ({})
  var lines = String(text || "").split("\n")
  for (var i = 0; i < lines.length; i++) {
    var m = /^(\d+):w:([01])$/.exec(lines[i].trim())
    if (m && Number(m[1]) < ids.length) results[ids[Number(m[1])]] = m[2] === "1"
  }
  return results
}
...
// Whether the apps/actions source toggles allow this entry anywhere in Beam
// (search, recents, the ✦ row and Jev's catalog).
function allowedBySources(entry, sources) {
  if (!entry || !sources) return false
  return entry.kind === "app" ? sources.apps !== false : sources.actions !== false
}
...
// Longer text is a paste, not a search: skip it rather than split thousands of
// words against every entry on each keystroke.
var MAX_SEARCH = 200

function search(items, entries, query, limit) {
  var q = String(query || "").trim()
  if (!q || q.length > MAX_SEARCH) return []
  var rows = []
...
// Hints go on actions only, and never on Install/Remove/Update entries: a
// keybinding opens the thing (Style › Theme), it does not install or remove it,
// and those entries share labels with the real one.
function hintAllowed(entry) {
  if (!entry || entry.kind !== "action") return false
  return !/^(install|remove|update)\./.test(String(entry.id))
}

// What Jev may pick from: apps and everyday actions. Install/Remove/Update
// entries reuse the labels of the real actions ("Bluetooth" under Update
// restarts the service) and are never what a description means; they stay
// findable by name.
function jevEligible(entry) {
  if (!entry) return false
  return !/^(install|remove|update)(\.|$)/.test(String(entry.id))
}
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    searchable: searchable, search: search, appItems: appItems, allowedBySources: allowedBySources, parseKeybindings: parseKeybindings, prettyKeys: prettyKeys,
    hintFor: hintFor, hintAllowed: hintAllowed, jevEligible: jevEligible
  }
}
